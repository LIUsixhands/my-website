...
from netlify_deploy import deploy_overlay, find_token  # noqa: E402
...
def set_date(path: Path, when: str):
    """把稿件的 date 對齊真正發布的那天。

    front matter 的 date 會變成 JSON-LD 的 datePublished。囤稿時寫的是「預計發布日」，
    實際發布常常會因為排程漏跑而延後，兩邊不一致就等於結構化資料在說謊。
    在編譯之前先改掉，產出的 HTML 就是對的。
    """
    text = path.read_text(encoding="utf-8")
    if re.search(r"^date:", text, re.M):
        text = re.sub(r"^date:.*$", f"date: {when}", text, count=1, flags=re.M)
...
TRANSIENT = ("Resource deadlock", "Errno 11", "Errno 35", "Errno 60",
             "Operation timed out", "Resource temporarily unavailable")
# 2026-09-13～18 連六天只重試 12 秒就放棄：09:07 剛開機登入時 iCloud 正忙著同步，
# 要等幾分鐘才會放手。改成遞增等待，最長約 8 分鐘。
BACKOFF = (5, 15, 30, 60, 120, 240)


def fail(msg):
    """失敗一定要跳通知。原本只寫 log，斷更六天沒人知道。"""
    log(f"✗ {msg}")
    notify("小搜 · 部落格發布失敗", msg[:120])
    raise SystemExit(1)


def run(cmd):
    """執行子指令，iCloud 暫時性錯誤會遞增等待後重試。
    ...
    for attempt in range(len(BACKOFF) + 1):
        ...
        last = f"{r.stdout}\n{r.stderr}"
        if attempt < len(BACKOFF) and any(k in last for k in TRANSIENT):
            log(f"  ⚠ iCloud 讀寫暫時失敗，{BACKOFF[attempt]} 秒後重試（第 {attempt + 2} 次）")
            time.sleep(BACKOFF[attempt])
            continue
        break
    log(f"指令輸出：\n{last}")
    fail(f"指令失敗：{Path(str(cmd[1])).name} {cmd[2] if len(cmd) > 2 else ''}")


def read_retry(path: Path) -> bytes:
    """讀 iCloud 桌面上的檔案，遇到暫時性錯誤同樣遞增重試。"""
    for attempt in range(len(BACKOFF) + 1):
        try:
            return path.read_bytes()
        except OSError as e:
            if attempt < len(BACKOFF) and any(k in str(e) for k in TRANSIENT):
                log(f"  ⚠ 讀 {path.name} 暫時失敗，{BACKOFF[attempt]} 秒後重試")
                time.sleep(BACKOFF[attempt])
                continue
            fail(f"讀不到 {path.name}：{e}")


def blog_overlay(slug):
    """這次部署要換上去的檔案：已發布文章＋本次這篇＋列表頁＋sitemap＋llms.txt。

    只挑已發布的 slug，是因為之前失敗的那幾輪會在 blog/ 留下草稿 html，
    不能順手把沒發布的東西推上線。內容先複製到本機暫存，避免上傳時再碰 iCloud。
    """
    slugs = {slug}
    for f in POSTS.glob("*.md"):
        meta, _ = read_front_matter(f)
        if meta.get("published") and meta.get("slug"):
            slugs.add(meta["slug"])
    rels = [f"blog/{s}.html" for s in sorted(slugs)] + \
           ["blog/index.html", "sitemap.xml", "llms.txt"]
    stage = Path(tempfile.gettempdir()) / "xiaosou_overlay"
    overlay = {}
    for rel in rels:
        src = SITE / rel
        if not src.exists():
            fail(f"部署前檢查：{rel} 不存在")
        dst = stage / rel
        dst.parent.mkdir(parents=True, exist_ok=True)
        dst.write_bytes(read_retry(src))
        overlay["/" + rel] = dst
    return overlay
...
def main():
    ...
    # 1) 沒稿就警告，不自動生
    if not pend:
        # plist 改成 RunAtLoad=true 之後，開機／登入都會叫一次，
        # 所以「沒稿」的通知要按天去重，否則同一天會連彈好幾次。
        today = date.today().isoformat()
        first_today = st.get("last_empty") != today
        if first_today:
    ...
    # 3) 編譯 + 重產 sitemap → 4) 只覆蓋部落格檔案到線上
    original = nxt["path"].read_text(encoding="utf-8")
    set_date(nxt["path"], date.today().isoformat())   # 先對齊日期再編譯
    py = sys.executable
    try:
        ...
        # 不再用 make_deploy 整站重打包（會刪掉 /logo/ 等根目錄路徑的檔案、
        # 還要讀 63MB 的影片 PDF 而被 iCloud 卡住）。只換部落格自己的檔案。
        overlay = blog_overlay(slug)
        res = deploy_overlay(overlay, NETLIFY_SITE_ID, netlify_token(), log=log)
        if res["state"] != "ready":
            fail(f"部署失敗：{res.get('state')} {res.get('error_message', '')}")
    except SystemExit:
        # 失敗就把 date 還原；不然每失敗一次這篇就被改成今天、排到別篇後面，佇列順序亂跳
        nxt["path"].write_text(original, encoding="utf-8")
        raise
    ...
    # 6) 驗證線上：文章本身要 200，列表頁也要真的列出這篇
    if not check_live(url):
        log(f"⚠ 部署成功但 {url} 抓不到 200，請人工確認（不視為失敗，可能是 DNS 或快取）")
    else:
        log(f"線上確認 200：{url}")
    try:
        idx = urllib.request.urlopen(f"{BASE}/blog/", timeout=30).read().decode("utf-8", "replace")
        if f"{slug}.html" not in idx:
            log(f"⚠ 線上列表頁沒有列出 {slug}，請人工確認")
            notify("小搜 · 列表頁沒更新", title)
    except Exception as e:
        log(f"⚠ 讀不到線上列表頁：{type(e).__name__}")
