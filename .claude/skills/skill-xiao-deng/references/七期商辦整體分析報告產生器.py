...
p2=f"""<div class="page"><div class="pad">
 ...
 <div style="font-size:7.4pt;color:#8a7a60;margin-top:1.5mm">
  單價＝淨單價（總價與面積皆已扣除車位），<b>一律只採民國 113–115 年（近三年）成交</b>——行情逐年變動，跨年平均會嚴重失準。成屋案另已排除建商交屋期那批低價；預售案（聯聚中維、市政壹號）為建商銷售成交價，不宜與成屋案直接比較。單價上排為<b>不含車位</b>（估價基準），下排為<b>含車位</b>（總價÷總面積，實登原始欄位與多數公開查詢平台採此口徑，兩者差 +0.8%～+10.4%）。坪數為扣車位後之權狀坪（<b>含公設</b>），採全部中古轉售樣本之中位；<b>典型總價＝單價 × 坪數，為房屋部分，車位另計</b>。
...
p4=f"""<div class="page"><div class="pad">
 ...
 <div style="margin-top:5mm" class="note">
  ...
  市政壹號 26 坪／80.6 萬，親家 T3 61 坪／40.3 萬，聯聚中雍 224 坪／67.4 萬。<br>
...
# ── P5 第四章 價格與總價帶 ──
PB=[("鼎盛BHW","≤104",231,28.51,50.31),("CBD時代廣場","≤104",252,28.88,48.27),
    ("親家T3市政國際中心","≤103",81,24.93,40.31),("親家T-POWER","≤102",49,28.32,43.92),
    ("NTC國家商貿中心","≤107",228,32.40,49.90),("豐邑市政都心廣場","≤102",8,27.46,36.90),
...
p5=f"""<div class="page"><div class="pad">
 ...
 <div style="margin-top:5mm" class="warn">
  ...
  <span class="b">含車位</span>＝ 總價 ÷ 總面積 ── <span class="b">實登原始單價欄位與多數公開查詢平台採此口徑</span><br>
  兩者最大差到 <span class="b">10.4%</span>（聯聚中維 73.5 vs 66.6），最小 +0.8%（親家 T-POWER）。車位席數越多、車位價越高，差越大。
 ...
 <div style="font-size:8.5pt;color:#6b5c46;line-height:1.8;margin-bottom:2mm">
  ...
  把交屋潮那批算進平均，會<span class="b">嚴重低估行情——最誇張的鼎盛 BHW 差到 1.76 倍</span>。
...
p6=f"""<div class="page"><div class="pad">
 ...
 七期商辦的毛投報率<span class="b">中位只剩 2.93%，七棟裡有四棟低於 3%</span>——
 ...
 <div class="note" style="margin-top:5mm">
  <span class="b g">◆ 為什麼掉下來？</span>近三年七期商辦售價漲了 <span class="b">1.2 至 1.8 倍</span>，
...
CASES=[
 ...
 ("鼎盛BHW","78 坪｜50.3 萬／坪｜典型總價 3,903 萬｜屋齡 12 年｜2.82%",
  ["<b>近三年漲幅全區最猛</b>：交屋潮 28.5 → 現行 50.3，達 1.76 倍",
   "<b>一案兩棟，產品互補</b>：A 棟坪數適中、B 棟坪數大","車位 828 席，<b>停車最充裕</b>","基地 1,641 坪，量體完整"],
  ["<b>近三年成交僅 7 筆</b>，行情帶寬須逐戶確認","毛投報 2.82%，<b>已低於 3%</b>",
   "屋齡 12 年，設備進入更新期","<b>實登社區名稱空白</b>、部分公開資料地址有誤 → 查價困難",
   "公設比 38.16%，中上水準"]),
 ("NTC國家商貿中心","73 坪｜49.9 萬／坪｜典型總價 3,654 萬｜屋齡 8 年｜3.12%",
  ["中階級距中<b>租金最強</b>（1,348 元／坪），投報 3.12% 為中階最佳",
   "73 坪為最泛用的中型坪數","另有大坪數戶（實登最大 838 坪），<b>擴充彈性大</b>","屋齡 8 年，設備仍新"],
  ["<b>樓層溢價僅 +0.4%</b> → 高樓層要溢價站不住腳（對買方是優勢、對屋主是壓力）",
   "交屋潮 32.4 → 現行 49.9，<b>達 1.54 倍</b>，看錯年份極易誤判","243 戶，出入較雜"]),
 ("CBD時代廣場","90 坪｜48.3 萬／坪｜典型總價 4,344 萬｜屋齡 10 年｜2.68%",
  ["<b>公設比 36.17% 為全區最低</b> → 每坪實際可用最多","90 坪中大型，格局選擇多，亦有 456 坪大戶",
   "朝馬／七期西側，周邊生活機能成熟"],
  ["<b>毛投報 2.68% 為全區最低</b>，租金追不上售價漲幅","<b>典型總價 4,344 萬為中階最高</b>",
   "255 戶，戶數偏多","交屋潮 28.9 → 現行 48.3，達 1.67 倍"]),
 ("親家T-POWER","69 坪｜43.9 萬／坪｜典型總價 3,013 萬｜屋齡 9 年｜2.93%",
  ["<b>中階級距的最低門檻</b>，約 3,013 萬","69 坪是市場最好用的中型坪數","176 戶相對單純","屋齡 9 年，中階中較新"],
  ["<b>28 樓為全區偏低</b>，視野與地標性較弱","毛投報 2.93%，<b>已低於 3%</b>",
   "近三年成交 13 筆，樣本中等","實登登記為「親家市政廣場」，<b>極易與親家 T3 混淆</b>"]),
 ("豐邑市政都心廣場","102 坪｜36.9 萬／坪｜典型總價 3,759 萬｜屋齡 16 年｜4.29%",
  ["<b>毛投報 4.29% 為全區最高</b>，且明顯高出第二名一大截","租金 1,322 元／坪，全區第二高",
 ...
 ("親家T3市政國際中心","61 坪｜40.3 萬／坪｜典型總價 2,463 萬｜屋齡 13 年｜2.89%",
  ["<b>典型總價 2,463 萬，成屋中進場門檻最低</b>","單價 40.3 萬／坪，成屋中最低",
   "61 坪適合 10–20 人團隊","樓層溢價 +11.5%，鈍感派中相對有差異"],
  ["租金 972 元／坪偏低，<b>毛投報 2.89% 低於 3%</b>","屋齡 13 年",
   "29 樓、市政路末端，<b>門面感相對弱</b>","交屋潮 24.9 → 現行 40.3，達 1.62 倍"]),
]
...
p9=f"""<div class="page"><div class="pad">
 ...
 <div class="note" style="margin-top:3mm">
  <span class="b g">◆ 中階這一層擠了五棟</span>，典型總價從 3,013 萬到 4,344 萬。
...
STEPS=[("①","先問坪數，不要先問預算",
       ...
       ("④","別把收租當主要理由",
        "用近三年成交重算，七棟毛投報 2.68–4.29%，<b>中位僅 2.93%，四棟低於 3%</b>。"
        "扣掉管理費、稅與空置後所剩無幾。<b>自用、門面與保值才是現在的買點。</b>"),
       ("⑤","查行情先問三件事：哪一年、扣不扣車位、幾筆",
        "交屋潮價與現行行情最多差 <b>1.76 倍</b>；實登原始單價欄只在車位有揭露時才扣；"
...
SCEN=[("10–20 人自用，預算 2,500 萬內","親家 T3 市政國際中心","成屋中門檻最低，61 坪、約 2,463 萬"),
      ("成長期企業，30–50 人，3,000–4,000 萬","親家 T-POWER、NTC、鼎盛 BHW","69–78 坪；此區間選擇最多，議價空間最大"),
      ("要大空間、坪效優先","CBD 時代廣場","90 坪，公設比 36.17% 全區最低；但投報 2.68% 全區最低"),
      ("收租為主","豐邑市政都心廣場","毛投報 4.29% 全區最高且大幅領先；但屋齡 16 年、樣本薄"),
...
p10=f"""<div class="page"><div class="pad">
 ...
 <div class="note" style="margin-top:6mm">
  <span class="b g">◆ 最後一句實話：</span>近三年七期商辦漲了 1.2–1.8 倍，投報率被壓到 3% 上下。
...
p11=f"""<div class="page"><div class="pad">
 ...
 <table style="margin-bottom:4mm;font-size:7.9pt">
  ...
  <tr><td class="b">① 扣除車位<br>（兩種都給）</td><td>實登「單價」欄<span class="b">只在車位價格有揭露時才扣車位</span>，口徑不一致。
   ...
   兩者相差 <span class="b">+0.8% 至 +10.4%</span>。實登原始欄位與多數公開查詢平台採含車位口徑，<span class="b">比價前務必先確認對方用哪一種</span>。
  ...
  <tr><td class="b">③ 分離交屋潮</td><td>新案交屋期的建商銷售成交與日後屋主轉售分列統計，
   兩者最大差距達 <span class="b">1.76 倍</span>（鼎盛 BHW：交屋潮 28.51 → 現行 50.31）。<span class="b">本報告一律只採屋主中古轉售組。</span></td></tr>
